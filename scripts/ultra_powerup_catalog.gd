class_name UltraPowerupCatalog
extends RefCounted

const DATA_PATH := "res://data/ultra_powerups.json"
...
static func load_catalog() -> void:
	...
	if typeof(parsed) != TYPE_ARRAY:
		push_error("UltraPowerupCatalog: failed to parse %s" % DATA_PATH)
		return
	_entries.clear()
	_by_id.clear()
...
static func random_id() -> String:
	...
	if _entries.is_empty():
		return ""
	return _entries[randi() % _entries.size()].get("id", "")
